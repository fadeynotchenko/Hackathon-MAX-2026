...
помощника), `MAX_MINI_APP_NAME` (имя мини-аппа для кнопки
...
- **GigaChat API** (`https://api.giga.chat/v1`, токен через
  `https://ngw.devices.sberbank.ru:9443/api/v2/oauth`) — модель помощника.
  Необязателен: без ключа помощник выключен, остальной сценарий работает.
  Модель только предлагает значения полей; проверяет их домен, файл собирает
  шаблонизатор, отправляет человек.
...
- Помощник на GigaChat работает через API (заполнение из сообщения, ответы по
  документу, сопроводительное письмо) и включается ключом `GIGACHAT_AUTH_KEY`;
  без ключа его ручки отвечают 503 `agent.unavailable`, а бот в чате отвечает,
  что помощник выключен. Бот пока понимает только текст: фото и голосовые — в
  планах.
- Шаблоны компании (загрузка своего DOCX) и распознавание фотографий не
  реализованы, см. [docs/PRODUCT.md](docs/PRODUCT.md).
