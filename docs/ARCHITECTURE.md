...
```
  MAX ──webhook/polling──▶ bot (Node) ──XADD──▶ Redis Stream to_core ──▶ core: api (воркер событий) ──▶ Postgres
                             ▲
                             └──XREADGROUP── Redis Stream to_bot ◀──XADD── core: api (/admin/notify)
...
```
        ...
        └───────────┘    └───────────┘         (initData, ссылки MAX, исключения)
```
...
Слои по именам: `api`, `usecases`, `events`, `db`, `logs`, `config`, `domain`.
