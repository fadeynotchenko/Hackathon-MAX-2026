...
Идемпотентность по `Event.id` — отметка в Redis в два шага: «в работе» (3 минуты)
на время обработчика и «сделано» (неделя) после него. Повтор события, которое
ещё «в работе», не подтверждается молча, а остаётся pending: если процесс убили
посреди ответа модели, повтор после истечения отметки выполнит работу. Нажатие
кнопки бота вдобавок сверяется по паре «пользователь + payload» в окне 5 секунд:
двойное нажатие «Прислать PDF» приходит двумя событиями с разными id.
