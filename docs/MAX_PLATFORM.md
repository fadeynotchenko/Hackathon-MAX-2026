...
- Разметка: приветствие, помощь и ответы помощника уходят с `format: 'html'`
  (`<b>`, `<i>`, `<code>`). Всё, что пришло от человека или модели, экранируется
  (`fmt.escapeHtml` в боте, `html.escape` в core). Слишком длинный ответ core
  обрезает и отправляет простым текстом: HTML, обрезанный посреди тега, MAX не примет.
  В апдейте нажатия кнопки текст приходит без HTML (разметка идёт отдельным списком
  `markup`), поэтому бот запоминает HTML сообщений с кнопками в Redis
  (`messages:markup:<mid>`, неделя). После нажатия кнопки снимаются, а жирный шрифт остаётся.
